...
string func_jian(string s1,string s2){
	int a[250]={0},b[250]={0},c[250]={0};string result="";
...
string func_jia(string s1,string s2){
	string result="";
	int a[2000]={0},b[2000]={0},c[2002]={0};
	for(int i=s1.length()-1;i>=0;i--){a[s1.length()-i-1]=s1[i]-'0';}
	for(int i=s2.length()-1;i>=0;i--){b[s2.length()-i-1]=s2[i]-'0';}
	for(int i=0;i<max((int)s1.length(),(int)s2.length());i++){c[i]+=a[i]+b[i];if(c[i]>=10){a[i+1]++;c[i]-=10;}}
...
int main(){
	string s1,s2,result="";int point_num;cin>>s1>>s2>>point_num;int len=s1.length();
	...
	result=result.substr(len)+".";len=s1.length();
	string result_point="";temp+=num[len]+'0';
	for(int i=len;i<=len+point_num-1;i++){
		int num_temp=find(temp,s2);result_point+=num_temp+'0';
		temp=func_jian(temp,x_num_list[num_temp]);temp=(temp=="0")?"":temp;temp+=num[i]+'0';
	}
	num_temp=find(temp,s2);result_point+=num_temp+'0';
	temp=func_jian(temp,x_num_list[num_temp]);len=0;
	if(*(result_point.end()-1)>='5' and temp!="0"){result_point=func_jia(result_point.substr(0,result_point.length()-1),"1");}
	else{result_point[result_point.length()-1]='\0';};//len=result_point.length()-2;
	//while(result_point[len]=='0' and len>=0){len--;};result_point=result_point.substr(0,len+1);
	cout<<result+result_point;return 0;
}
